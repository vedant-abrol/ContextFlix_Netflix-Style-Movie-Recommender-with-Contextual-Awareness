...
# Instead of using @app.before_first_request which is deprecated
    ...
    try:
        ...
        for user_id in users[:100]:  # Limit to first 100 users for performance
            try:
                ...
                avg_rating = float(user_data['Rating'].mean())  # Ensure avg_rating is a float
                ...
                try:
                    if 'Gender' in user_data.columns:
                        demographics['gender'] = str(user_data['Gender'].iloc[0])
                    
                    if 'Age' in user_data.columns:
                        demographics['age'] = int(float(user_data['Age'].iloc[0]))  # Convert to int safely
                    
                    if 'Occupation' in user_data.columns:
                        demographics['occupation'] = int(float(user_data['Occupation'].iloc[0]))  # Convert to int safely
                    
                    if 'Zip-code' in user_data.columns:
                        demographics['zip_code'] = str(user_data['Zip-code'].iloc[0])
                except Exception as e:
                    print(f"Error processing demographics for user {user_id}: {str(e)}")
        ...
        import traceback
        print(f"Error in /api/users endpoint: {str(e)}")
...
@app.route('/api/ab_test_results', methods=['GET'])
def ab_test_results():
    """Get the results of A/B testing."""
    if recommender is None:
        return jsonify({"error": "Model not initialized"}), 503
    
    try:
        # Check if we have A/B test results saved
        results_path = os.path.join(MODEL_DIR, 'ab_test_results.json')
        if os.path.exists(results_path):
            with open(results_path, 'r') as f:
                results = json.load(f)
            return jsonify(results)
        
        # If not saved, run A/B testing
        results = recommender.evaluate_with_ab_testing(test_ratio=0.2)
        
        # Save results for future use
        recommender.save_ab_test_results(results, results_path)
